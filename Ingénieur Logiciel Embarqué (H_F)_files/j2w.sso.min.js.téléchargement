...
""):""},forwardToApply:function(){j2w.SSO.$stack.trigger("close");j2w.Apply.countApplyGoToJob(j2w.SSO.getJobID())},localeFormattedMessageForFieldLabel:function(a,b){return jsStr[a].format(j2w.SSO.fieldLabelGetter(b)).unescapeHTML()},logoutHandler:function(a){void 0==j2w.SAB&&(document.location.href=window.location.host);j2w.SAB.email=void 0;null!=j2w.SAB.$activeButton&&j2w.SAB.$activeButton.find(".emailGetter").find("input[type=email]").val(void 0);j2w.SSO.start()},resetPasswordDone:function(a){j2w.SSO.$stack.find("form:visible").before('<div class="resetMessage alert alert-info">'+
